...
> - **出门在外（真正的随时随地）**：强烈推荐在电脑和手机上都安装 [Tailscale](https://tailscale.com/)，组建虚拟局域网。这样哪怕你在地铁上用 5G，也能连上家里的电脑！

1. 确保网络连通后，在手机端 APP 的首页，输入电脑上 `cdp-relay` 显示的 IP 地址和端口（如果是 Tailscale，请输入类似 `100.106.x.x:19336`）。
2. 选择你想操控的 IDE 窗口，进入沉浸式开发界面：
