...
async function autoLaunchWithCdp({ force = false, port = CDP_PORT, appName = '', cwd = '' } = {}) {
    ...
    for (const app of appsToTry) {
        try {
            ...
            // 判断是否已有该 app 的实例在运行
            // 预制端口：9333-9336 已预先复制了登录数据，可直接启动
            const PREPROVISIONED_PORTS = [9333, 9334, 9335, 9336];
            const isPreprovisioned = PREPROVISIONED_PORTS.includes(port);
            let launchCmd;

            if (!running && port === 9333) {
                ...
                if (isPreprovisioned && fs.existsSync(path.join(userDataDir, 'User'))) {
                    log(`📋 使用预制数据目录: ${userDataDir} (端口 ${port})`);
                } else if (!fs.existsSync(path.join(userDataDir, 'User')) && fs.existsSync(path.join(defaultDataDir, 'User'))) {
...
const server = http.createServer(async (req, res) => {
    ...
    // ── /cwd_history (目录历史管理，不区分应用) ──
    if (req.url.startsWith('/cwd_history')) {
        const parsed = urlModule.parse(req.url, true);
        
        if (req.method === 'GET') {
            const history = loadCwdHistory();
            res.writeHead(200, { 'Content-Type': 'application/json', ...cors });
    ...
    // ── /version (OTA)：此中继不提供 OTA，仅 relay-dev:19336 提供 ──
    if (req.url === '/version') {
        res.writeHead(200, { 'Content-Type': 'application/json', ...cors });
        res.end(JSON.stringify({ error: 'OTA disabled on this relay' }));
        return;
    }
    ...
    if (req.url.startsWith('/launch')) {
        ...
        if (activeCdpTargets.has(reqPort)) {
            ...
            if (reqCwd) {
                ...
                addCwdHistory(reqCwd, reqApp || t.appName); // 记录目录历史
        ...
        if (reqCwd) addCwdHistory(reqCwd, reqApp); // 记录目录历史
