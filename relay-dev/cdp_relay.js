                        ...
                        (function() {
                            ...
                            function isApproval(text) {
                                ...
                                return lower === 'run'
                                    ...
                                    || lower === 'continue'
                                    || lower === 'yes'
                                    || lower === 'yes, allow this time'
                                    || lower === 'always allow'
                                    || lower === 'allow once'
                                    || lower === 'allow in workspace'
                                    || lower === 'allow this time'
                                    || lower === 'approve action'
                                    || lower === 'run action'
                                    || lower.indexOf('allow running this command') >= 0
                                    || (lower.indexOf('yes') === 0 && lower.indexOf('allow') >= 0)
    ...
    if (req.url.startsWith('/codex/workspace/add')) {
        ...
        try {
            ...
            if (activeCdpTargets.has(reqPort)) {
                ...
                if (runtimeAddResult !== 'ok' && runtimeAddResult !== 'missing-electronBridge') {
                    res.end(JSON.stringify({
                        success: false,
                        port: reqPort,
                        appName: 'Codex',
...
function selectIdePage(pages, appName = '') {
    ...
    if (!page && (lower === 'dsme' || lower.includes('deepseek') || lower === 'antigravity' || lower.includes('antigravity'))) {
        ...
        (function() {
            ...
            for (var di = 0; di < roots.length; di++) {
                ...
                for (var i = 0; i < nodes.length; i++) {
                    ...
                    if (['run','allow','approve','continue','yes','yes, allow this time','always allow','allow once','allow in workspace','allow this time','approve action','run action'].indexOf(text) >= 0 ||
                        text.indexOf('allow running this command') >= 0 ||
                        (text.indexOf('yes') === 0 && text.indexOf('allow') >= 0)) {
                        return true;
                    }
                }
