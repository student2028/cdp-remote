                ...
                try {
                    ...
                    if (isMainCommit) {
                        ...
                        const prompt = [
                            ...
                            ``,
                            `⚠️ 核心规则（非常重要）：`,
                            `- 只需在对话框正常输出上述包含 VERDICT 的审查结果即可。`,
                            `- Relay 系统会自动解析你的结论并推进流水线，你不需要也不允许去终端执行任何 shell 命令。`,
                            ``,
                        ...
                        const prompt = [
                            ...
                            `3. 在回复末尾用以下固定格式输出第一步任务：`,
                            ``,
                            `---TASK_START---`,
                            `<第一步的完整任务描述，包含：目标、技术要求、完成标准>`,
                            `---TASK_END---`,
                            ``,
                            `⚠️ 重要规则：`,
                            `- 只在对话框中输出以上内容，不要执行任何 shell 命令（如 orchestra）`,
                            `- 不要自己写代码，你的职责是统筹和拆解`,
                            `- relay 系统会自动读取 ---TASK_START--- 到 ---TASK_END--- 之间的内容派发给工人`,
                        ].join('\n');
                        await sendWithRetry(port, prompt, 'brain');
